...
export class SaveSoundTaskFile {
    private readonly logger = new Logger(SaveSoundTaskFile.name);
...
export class SaveSoundTaskFileExist {
    private readonly logger = new Logger(SaveSoundTaskFileExist.name);
    ...
    async handle(channel: any, exchange: string) {
        ...
        await channel.consume(q.queue, async (msg: any) => {
            ...
            try {
                ...
                const filePath = content.filePath || "-";
                const fileName = content.fileName || path.basename(filePath);
                const info = content.info || path.basename(filePath);
...
export class SaveSoundTaskFileEvent {
    private readonly logger = new Logger(SaveSoundTaskFileEvent.name);
