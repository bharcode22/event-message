...
export class RabbitmqListenerService implements OnApplicationBootstrap {
    ...
    async onApplicationBootstrap() {
        ...
        const listeners = [
            { exchange: process.env.CREATE_TASK2_EXCHANGE, handler: new CreateTaskListener(this.telegramService) },
            { exchange: process.env.DELETE_TASK2_EXCHANGE, handler: new DeleteTaskListener(this.telegramService) },
            { exchange: process.env.ADMIN_INFORMATION_EXCHANGE, handler: new AdminInfoListener(this.telegramService) },
        ].filter(l => l.exchange);
